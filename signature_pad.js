...
var SignaturePad = (function (document) {
    ...
    var SignaturePad = function (canvas, options) {
        ...
        this.backgroundColor = opts.backgroundColor || 'white';// "rgba(0,0,0,0)";
		this.bAutoResize = opts.bAutoResize || false;
		...
		this.resize=function() {
			...
			if (self.lastRatio!=ratio) {
				self._canvas.width = self._canvas.offsetWidth * ratio;
				self._canvas.height = self._canvas.offsetHeight * ratio;
				self._canvas.getContext("2d").scale(ratio, ratio);
		...
		if (this.bAutoResize) this.resize();
        this.clear();
    ...
    SignaturePad.prototype={
		...
		toDataURL : function (imageType, quality) {
			return this._canvas.toDataURL(imageType,quality);
		},
		...
		_xForm : function (rect,px) { 
			return this._canvas.width*px/(rect.right-rect.left);
		},
	
		_yForm : function (rect,py) { 
			return this._canvas.height*py/(rect.bottom-rect.top);
		},
		...
		_createPoint :function (event) {
			...
			return new Point(
				this._xForm(rect,event.clientX - rect.left),
				this._yForm(rect,event.clientY - rect.top)
			);
		},
